...
ClapTrap::ClapTrap(std::string val, int att_damage, int h_point, int e_point)
	:name(val), attack_damage(att_damage), hit_point(h_point), energy_point(e_point)
{
...
void	ClapTrap::attack(const std::string& target)
{
	if (hit_point > 0 && energy_point > 0)
	{
		...
		std::cout << "ClapTrap " << name << " has no hit points or engergy points left.\n";
}
void	ClapTrap::takeDamage(unsigned int amount)
{
	if ((unsigned int)hit_point < amount)
	{
		std::cout << "ClapTrap " << name << " has no hit points or engergy points left.\n";
		hit_point = 0;
	}
	else if ((hit_point - amount) >= 0 && energy_point > 0)
	{
